...
export default function Dashboard() {
  ...
  const totalProjectValue = projects.reduce((total, project) => total + project.value, 0);

  const projectsCompleted = projects.filter((project) => project.status === "Completed").length;

  const projectsInProgress = projects.filter((project) => project.status === "In Progress").length;
